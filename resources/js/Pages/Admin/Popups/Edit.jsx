...
export default function Index({ popup }) {
    ...
    const { data, setData, post, processing, errors, recentlySuccessful } = useForm({
        title: popup?.title || '',
        message: popup?.message || '', // Pastikan field ini sesuai dengan kolom di database
        is_active: popup?.is_active ?? false,
        image_url: popup?.image_url || '',
        _method: 'PUT', // Spoofing agar Laravel mengenali POST ini sebagai PUT
    });

    const submit = (e) => {
        e.preventDefault();
        
        // Pastikan menyertakan popup.id agar Laravel tahu data mana yang diupdate
        post(route('admin.popups.update', popup.id), {
            preserveScroll: true,
            forceFormData: true, // Memaksa pengiriman sebagai FormData
        });
    };
